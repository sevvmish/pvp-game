   ...
   public class TextureArrayConfigEditor : Editor
   {
      ...
      void DrawHeader(TextureArrayConfig cfg)
      {
         if (cfg.textureMode != TextureArrayConfig.TextureMode.Basic)
         {
            ...
            else
            {
               ...
               EditorGUILayout.LabelField (cfg.IsDecal () ? CAlpha : CAO, GUILayout.Width (64));
      ...
      bool DrawTextureEntry(TextureArrayConfig cfg, TextureArrayConfig.TextureEntry e, int i, bool controls = true)
      {
         ...
         EditorGUILayout.EndVertical();
            ...
            if (cfg.textureMode != TextureArrayConfig.TextureMode.Basic || cfg.IsScatter () || cfg.IsDecal ())
            {
               EditorGUILayout.BeginVertical ();
               if (controls)
               {
         ...
         bool isCluster = false)
      {
         ...
         // splat
         var spalcmp = GetTextureFormat (cfg, settings.decalSplatSettings.compression);
         ...
         for (int i = 0; i < src.Count; ++i)
         {
            try
            {
               ...
               {
                  EditorUtility.CompressTexture (splatTex, GetTextureFormat (cfg, settings.decalSplatSettings.compression), tq);
               }
